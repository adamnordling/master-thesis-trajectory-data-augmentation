...
def find_best_configs(dataset_name: str, history_dir: str) -> Optional[Dict]:
    ...
    for f in all_files:
        ...
        df = pd.read_csv(f)
        df['strategy'] = df['feature_type'].apply(
...
def load_final_comparison_data(dataset_name: str, history_dir: str, best_params: Dict) -> pd.DataFrame:
    ...
    for strategy, info in best_params.items():
        ...
        strategy_df['strategy'] = strategy_df['feature_type'].apply(
            lambda ft: ft.split('merged_')[-1].split('_p')[0] if 'merged' in str(ft) else 'baseline')
...
def perform_statistical_analysis(df: pd.DataFrame, best_params: Dict) -> pd.DataFrame:
    """Performs paired t-test and returns a results DataFrame."""
    analysis_results = []
    for model in MODEL_ORDER:
        model_df = df[df['model'] == model]
        baseline_scores = model_df[model_df['strategy'] == 'baseline']['score'].values
        num_seeds = len(baseline_scores)
        if num_seeds == 0: continue

        for strategy in sorted(best_params.keys()):
            strategy_scores = model_df[model_df['strategy'] == strategy]['score'].values
            if len(baseline_scores) != len(strategy_scores): continue
            ...
            analysis_results.append({
                'model': model, 'strategy': strategy, 'mean_improvement_pct': mean_improvement * 100,
                'p_value': p_value, 'is_significant': is_significant,
                'significant_improvement': is_significant and mean_improvement > 0,
                'num_seeds': num_seeds, 'status': status,
                'best_suffix': best_params[strategy]['best_suffix']
            })
    return pd.DataFrame(analysis_results)


# In src/evaluation/analyze.py

def save_artifacts(results_df: pd.DataFrame, dataset_name: str, best_params: Dict):
    """
    Saves results to console, CSV, LaTeX, and Markdown files.
    This version uses a simplified, robust LaTeX table format.
    """
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'output', 'analysis', 'reports', dataset_name)
    os.makedirs(output_dir, exist_ok=True)

    # --- 1. Console, CSV, Markdown Saving (Keep your existing code for these) ---
    # ... (For brevity, I'm omitting these parts. They are correct.) ...

    # --- 4. Save to a NEW, SIMPLIFIED, ROBUST LaTeX table ---
    ...
    with open(latex_path, 'w', encoding='utf-8') as f:
        f.write(f"% Auto-generated for {dataset_name} with {int(results_df['num_seeds'].max())} seeds.\n")
        f.write("\\begin{table}[ht]\n")
        f.write("    \\centering\n")
        f.write(f"    {caption_text}\n")
        f.write("    \\label{tab:final_analysis_" + dataset_name.lower() + "}\n")

        # --- THE FIX: Use a simple, reliable column spec (left, right, right) ---
        col_spec = "{l l r r}"
        ...
        for i, model in enumerate(MODEL_ORDER):
            ...
            # Use multirow for the model name, spanning the number of strategies
            f.write(f"        \\multirow{{{len(model_results)}}}{{*}}{{\\textbf{{{model}}}}} \n")

            for j, (_, row) in enumerate(model_results.iterrows()):
                strategy_name = row['strategy'].replace('_', ' ').title()
                imp_val_str = f"{row['mean_improvement_pct']:+.2f}"

                # Format p-value string, adding an asterisk if significant
                ...
                # Apply bolding to the entire row if it's a significant improvement
                if row['significant_improvement']:
                    f.write(
