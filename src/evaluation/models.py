...
def get_model_definitions(
    ...
    # We must map the clean YAML keys to the specific Scikit-Learn param names.
    # For Pipelines, this means adding the 'stepname__' prefix.
    ...
    if 'MLP' in raw_grids:
        param_grids['MLP'] = {
            f'classifier__{k}': v for k, v in raw_grids['MLP'].items()
        }
    ...
    if 'LogisticRegression' in raw_grids:
        param_grids['LogisticRegression'] = {
            f'classifier__{k}': v for k, v in raw_grids['LogisticRegression'].items()
        }
...
def train_and_evaluate(
    ...
    Args:
        model_config: The loaded dictionary from models.yaml.
    """
    ...
    for model_name, model_instance in models.items():
        try:
            ...
            if not cache_hit.empty:
                ...
                # Case B: Baseline Tuning (Grid Search)
                logger.info(f"Tuning {model_name} for seed {seed}...")

                # Check if we have a grid for this model
                if model_name not in param_grids:
                    logger.warning(f"No grid defined in config for {model_name}. Using defaults.")
                    grid = {}
                else:
                ...
                # Case C: Missing Baseline Params for Augmented Run
